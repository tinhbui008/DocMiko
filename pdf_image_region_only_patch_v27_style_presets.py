...
# -*- coding: utf-8 -*-
"""
V27 Style Preset Engine for Image/Graphic-Region PDF Patching
...
def draw_textbox_fit(
    ...
    # Final fallback: draw at min size. For tiny graphic labels, better to see a small translation than leave English.
    page.insert_text(
        fitz.Point(rect.x0 + 0.5, rect.y0 + max(3.0, min_size)),
        text,
        fontsize=min_size,
        fontname=fontname,
        fontfile=fontfile,
...
def patch_image_regions_with_styles(
    ...
    print("V27 style-preset image/graphic-region patch summary:")
    ...
    print("  note=Patch style comes from presets first, then patch-level overrides. Text-layer outside image/graphic regions was preserved.")
