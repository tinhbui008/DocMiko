...
Pipeline:
  Step 1: python pdf_generator_single_anthropic_merged_v16_full_ocr_rebuild.py in.pdf clean.pdf ...
  Step 2: python v26_3_patch.py clean.pdf final.pdf --patch-map patch_map.json --source-pdf in.pdf
"""
...
def default_font_paths(font=None, bold=None, title=None):
    regular = first_existing(font, "fonts/NotoSans-Regular.ttf",
        r"C:\Windows\Fonts\arial.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf")
    bold_path = first_existing(bold, "fonts/NotoSans-Bold.ttf",
        r"C:\Windows\Fonts\arialbd.ttf", "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", regular)
...
def auto_image_rects(source_pdf: str, min_area: float = 500.0) -> Dict[int, List[fitz.Rect]]:
...
def rect_allowed(rect: fitz.Rect, regions: List[fitz.Rect],
                 mode: str = "center", threshold: float = 0.30) -> bool:
    if not regions:
        return False
...
def sample_bg(page: fitz.Page, rect: fitz.Rect, dpi: int = 140) -> RGB:
    try:
        ...
        pts = [
            (x0-pad, y0-pad), (x1+pad, y0-pad), (x0-pad, y1+pad), (x1+pad, y1+pad),
            ((x0+x1)//2, y0-pad), ((x0+x1)//2, y1+pad),
            (x0-pad, (y0+y1)//2), (x1+pad, (y0+y1)//2),
        ]
        vals = [img.getpixel((x,y)) for x,y in pts if 0<=x<w and 0<=y<h]
        if not vals:
            return (1, 1, 1)
...
def draw_round_rect(page: fitz.Page, rect: fitz.Rect, fill: RGB,
                    radius: Optional[float] = None, overlay: bool = True):
    if radius is None:
    ...
    for ox, oy in [(0,0),(rect.width-2*radius,0),(0,rect.height-2*radius),(rect.width-2*radius,rect.height-2*radius)]:
        page.draw_oval(fitz.Rect(rect.x0+ox, rect.y0+oy, rect.x0+ox+2*radius, rect.y0+oy+2*radius),
                       color=None, fill=fill, overlay=overlay)


def draw_textbox_fit(page: fitz.Page, rect: fitz.Rect, text: str,
                     fontfile: Optional[str], fontname: str,
                     fontsize: float, color: RGB, align: str):
    align_map = {"left": fitz.TEXT_ALIGN_LEFT, "center": fitz.TEXT_ALIGN_CENTER,
    ...
    while size >= min_size:
        ...
        if rc >= 0:
            return True
        size -= 0.2
    page.insert_text(fitz.Point(rect.x0+0.5, rect.y0+max(3.8, min_size)), text,
                     fontsize=min_size, fontname=fontname, fontfile=fontfile,
                     color=color, overlay=True)
    return False


def patch_image_regions_only(
    clean_pdf: str, output_pdf: str, patch_map_path: str,
    source_pdf: Optional[str] = None, image_region_map: Optional[str] = None,
    font: Optional[str] = None, font_bold: Optional[str] = None,
    font_title: Optional[str] = None,
    use_auto_image_rects: bool = True,
    dry_run: bool = False, debug_regions: bool = False,
):
    patches = load_patch_map(patch_map_path)
    auto_regions = auto_image_rects(source_pdf, min_area=500.0) if (use_auto_image_rects and source_pdf) else {}
    ...
    for page_index, page in enumerate(pdf):
        ...
        for p in patches:
            ...
            fill_mode = str(p.get("fill", "sample")).lower() if isinstance(p.get("fill"), str) else "explicit"
            bg = sample_bg(page, rect)

            fill_val = p.get("fill")
            if fill_val is None or (isinstance(fill_val, str) and fill_val.lower() == "none"):
                pass  # no fill
            elif isinstance(fill_val, str) and fill_val.lower() == "sample":
                fill = bg
                shape = str(p.get("shape", "rect")).lower()
                if shape in {"pill", "round", "rounded"}:
                    draw_round_rect(page, rect, fill=fill, radius=p.get("radius"), overlay=True)
                else:
                    page.draw_rect(rect, color=None, fill=fill, overlay=True)
                bg = fill
            else:
                fill = rgb01(fill_val, default=bg)
                shape = str(p.get("shape", "rect")).lower()
                if shape in {"pill", "round", "rounded"}:
                    draw_round_rect(page, rect, fill=fill, radius=p.get("radius"), overlay=True)
                else:
