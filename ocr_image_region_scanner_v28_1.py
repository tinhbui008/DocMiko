...
# -*- coding: utf-8 -*-
"""
V28.1 OCR Image-Region Scanner for PDF translation patches.
...
def init_paddle(lang: str = "en", use_angle_cls: bool = True):
    ...
    # PaddleOCR has changed parameters across versions. Try the common signatures.
    try:
...
def scan_regions(
    ...
    ocr = init_paddle(lang=lang, use_angle_cls=use_angle_cls)
        ...
        for ridx, (region_id, rect) in enumerate(regions[page_num], start=1):
            ...
            for variant_name, vimg in variants.items():
                ...
                try:
                    ...
                    print(f"WARNING: OCR failed page={page_num} region={region_id} variant={variant_name}: {e}", file=sys.stderr)
                    continue
