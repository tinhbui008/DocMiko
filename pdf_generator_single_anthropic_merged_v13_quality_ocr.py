...
# -*- coding: utf-8 -*-
"""
pdf_generator_single_anthropic_merged_v13_quality_ocr.py
...
Run:
    python pdf_generator_single_anthropic_merged_v13_quality_ocr.py input.pdf output.pdf `
...
# VERSION_MARKER = 'merged_v13_quality_ocr'
# ==================== V8 CORE ====================
import argparse
import copy
import json
...
def apply_cxview_manual_patches_v13(
    ...
    This is intentionally optional. It handles image/raster labels that OCR
    either misses or renders too poorly, without running broad OCR over the page.
    """
    pdf = fitz.open(pdf_path)
    font_regular = first_existing_font_v10(font_regular, font_bold)
    font_bold = first_existing_font_v10(font_bold, font_regular)

    # Page indexes are 0-based. All coords are PDF points for 841.89x595.28 pages.
    patches = [
        # Page 4 top product title and feature buttons.
        # Keep only small raster labels; do not touch already translated body text.
        (3, (333, 72, 510, 92), "Hộp CXVIEW GPT", None, (0.12, 0.10, 0.18), 14, "bold"),
        (3, (335, 94, 508, 111), "Kỷ nguyên mới của AI biên vật lý", None, (0.12, 0.10, 0.18), 8.5, "regular"),
        (3, (662, 72, 794, 91), "ĐỘ TRỄ GẦN 0", (0.45, 0.23, 0.64), (1, 1, 1), 7.5, "bold"),
        (3, (662, 96, 794, 115), "CHỦ QUYỀN DỮ LIỆU", (0.45, 0.23, 0.64), (1, 1, 1), 7.2, "bold"),
        (3, (662, 120, 794, 139), "TỐI ƯU CHI PHÍ", (0.45, 0.23, 0.64), (1, 1, 1), 7.4, "bold"),
        (3, (662, 144, 794, 163), "TIẾT KIỆM BĂNG THÔNG", (0.45, 0.23, 0.64), (1, 1, 1), 6.8, "bold"),

        # Page 5 small diagram captions. Use compact labels and avoid touching large sections.
        (4, (427, 255, 540, 272), "Giám sát thống kê dashboard", None, (0.12, 0.10, 0.18), 5.8, "regular"),
        (4, (590, 255, 720, 272), "Nhân sự được ủy quyền", None, (0.12, 0.10, 0.18), 5.8, "regular"),
    ]
    ...
    for page_index, box, label, fill, color, fontsize, weight in patches:
        ...
        if fill is not None:
            ...
            # For white/background patches, cover a tiny rectangle only behind text.
            if page_index == 3 and box[0] < 520:
    ...
    pdf.save(output_pdf, garbage=4, deflate=True)
    pdf.close()
    print(f"CXVIEW manual patches applied: {output_pdf}")
