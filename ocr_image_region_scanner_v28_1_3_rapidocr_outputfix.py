...
# -*- coding: utf-8 -*-
"""
V28.1.3 OCR Image-Region Scanner for PDF translation patches.
...
def normalize_ocr_result(raw: Any) -> List[Tuple[List[List[float]], str, float]]:
    ...
    Supports:
    - PaddleOCR legacy list output
    - PaddleOCR 3.x dict output
    - RapidOCR legacy tuple/list output
    - RapidOCR 2.x RapidOCROutput object: .boxes / .txts / .scores

    The v28.1.2 scanner missed RapidOCR 2.x object output and therefore
    produced raw_candidates=0 even when the engine detected text.
    """
    out: List[Tuple[List[List[float]], str, float]] = []
    if raw is None:
        return out

    # RapidOCR 2.x returns a RapidOCROutput-like object with attributes
    # boxes (N,4,2), txts (N), scores (N). Support it before list/dict checks.
    if all(hasattr(raw, attr) for attr in ("boxes", "txts", "scores")):
        try:
            boxes = getattr(raw, "boxes") or []
            texts = getattr(raw, "txts") or []
            scores = getattr(raw, "scores") or []
            for box, text, score in zip(boxes, texts, scores):
                quad = box_to_quad(box.tolist() if hasattr(box, "tolist") else box)
                if quad and text:
                    out.append((quad, str(text), float(score)))
            return out
        except Exception:
            # Fall through to other formats if attribute parsing failed.
            pass

    # Some wrappers expose a dict-like/result object via .to_dict(), .dict(), or .as_dict().
    ...
    # New PaddleOCR / dict-like outputs.
    if isinstance(raw, dict):
        texts = raw.get("rec_texts") or raw.get("txts") or raw.get("texts") or []
        scores = raw.get("rec_scores") or raw.get("scores") or []
        boxes = raw.get("rec_boxes") or raw.get("boxes") or raw.get("dt_boxes") or []
        for box, text, score in zip(boxes, texts, scores):
            quad = box_to_quad(box.tolist() if hasattr(box, "tolist") else box)
            if quad and text:
                out.append((quad, str(text), float(score)))
    ...
    for item in candidates:
        try:
            ...
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                ...
                # RapidOCR legacy style: [box, text, score]
...
def scan_regions(
            ...
            for variant_name, vimg in variants.items():
                ...
                for quad, text, score in normalize_ocr_result(raw):
                    ...
                    region_items.append(OCRItem(
                        ...
                        detection_count=1,
                        english_like=True,
                        source=f"{active_engine_name}_v28_1_3",
                    ))
    ...
    pdf.close()
    report = {
        "version": "v28_1_3_ocr_image_region_report",
        "input_pdf": pdf_path,
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print("V28.1.3 OCR Image-Region Scanner summary:")
