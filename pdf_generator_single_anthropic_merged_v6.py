...
# -*- coding: utf-8 -*-
"""
pdf_generator_single_anthropic_userfix.py
...
Run:
    python pdf_generator_single_anthropic_userfix.py input.pdf output.pdf `
...
# VERSION_MARKER = 'userfix_compile_anthropic'
# ==================== V8 CORE ====================
import argparse
import copy
import json
...
def is_translatable_block(block: TextBlock, translate_headers_footers: bool = False) -> bool:
    ...
    # Slide decks often classify real titles as "header" because they sit near page top.
    # Allow headers when we have deterministic memory for them.
    if block.role in {"header", "footer"} and not translate_headers_footers:
        key = normalize_translation_key(block.original_text)
        memory_keys = set(EXACT_TRANSLATION_MEMORY_VI.keys())
        try:
            memory_keys |= set(COMPACT_TRANSLATION_MEMORY_VI.keys())
        except Exception:
            pass
        ...
        upper = block.original_text.strip().upper()
        if "CXVIEW SMART AI VIDEO ANALYTICS SOLUTION" in upper:
            return False
        return False
...
def draw_layout_v6(page: fitz.Page, layout: LayoutResult, block: TextBlock, resolver: FontResolverV6, color: RGB):
    """Draw translated layout using weight-aware block font."""
    if not layout.lines:
        return
    ...
    for line in layout.lines:
        ...
        if layout.align == "center":
            w = measure_font.text_length(measure_line, fontsize=layout.fontsize)
            x = x0 + max(0, (layout.rect.width - w) / 2)
        elif layout.align == "right":
            w = measure_font.text_length(measure_line, fontsize=layout.fontsize)
            x = x1 - w
...
def main_v6(argv: Optional[List[str]] = None):
    ...
    if args.preview_dir:
        print(f"[5/5] Render preview PNGs: {args.preview_dir}")
