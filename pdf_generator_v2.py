...
# -*- coding: utf-8 -*-
"""
pdf_generator_v2_babeldoc_like.py
...
Usage:
    python pdf_generator_v2_babeldoc_like.py test_vietnamese.pdf output_v2.pdf

With prepared translation map:
    python pdf_generator_v2_babeldoc_like.py test_vietnamese.pdf output_v2.pdf --translation-map translated.json

With custom Vietnamese fonts:
    python pdf_generator_v2_babeldoc_like.py test_vietnamese.pdf output_v2.pdf --font fonts/NotoSans-Regular.ttf --font-bold fonts/NotoSans-Bold.ttf --font-title fonts/NotoSansCondensed-Bold.ttf
"""
...
class FontResolver:
    def __init__(
        ...
        self.regular_font = self._first_existing([regular_font, *self._windows_font_candidates("regular")])
        self.bold_font = self._first_existing([bold_font, *self._windows_font_candidates("bold")])
        self.title_font = self._first_existing([title_font, *self._windows_font_candidates("title"), self.bold_font, self.regular_font])
    ...
    @staticmethod
    def _windows_font_candidates(kind: str) -> List[str]:
        base = Path("C:/Windows/Fonts")
        if kind == "title":
            return [
                str(base / "arialnb.ttf"),  # Arial Narrow Bold
                str(base / "ARIALNB.TTF"),
        ...
        if kind == "bold":
            return [
                str(base / "arialbd.ttf"),
                str(base / "ARIALBD.TTF"),
                str(base / "calibrib.ttf"),
                str(base / "CALIBRIB.TTF"),
            ]
        return [
            str(base / "arial.ttf"),
            str(base / "ARIAL.TTF"),
            str(base / "calibri.ttf"),
            str(base / "CALIBRI.TTF"),
        ]
    ...
    def fontname_for(self, block: TextBlock) -> str:
        # If fontfile is provided, fontname just needs to be a valid internal name.
        return self.fallback_fontname

    def fitz_font_for(self, block: TextBlock) -> fitz.Font:
        fontfile = self.fontfile_for(block)
        key = fontfile or self.fallback_fontname
