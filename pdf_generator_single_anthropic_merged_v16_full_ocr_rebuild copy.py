...
# ============================================================
# Uses Claude's vision capability to read all visible text from
# a page image — understands layout context, handles colored
# buttons, embedded image text, and complex backgrounds that
# Tesseract/PaddleOCR miss entirely.
#
# How it works:
#   1. Render PDF page to PNG at high DPI
#   2. Send image to Claude claude-haiku-4-5 (cheap, fast) with a
#      structured prompt requesting JSON: [{text, bbox, role}]
#   3. Claude returns ALL visible text with approximate bounding
#      boxes as fractions of page dimensions
#   4. Convert fractional bbox → PDF points
#   5. Same cover+draw pipeline as Tesseract/Paddle
#
# Cost estimate: claude-haiku-4-5 ~$0.001–0.003 per page image
# (much cheaper than GPT-4o, comparable to Mistral OCR)


def _claude_vision_page_to_b64(page: fitz.Page, dpi: int = 200) -> str:
    """Render PDF page to base64 PNG for Claude Vision API."""
    import base64
...
def _claude_vision_call(
    ...
    """Call Claude Messages API with an image and return text response."""
    prompt = """You are a precise OCR engine for PDF slides. 
Analyze this PDF page image and extract ALL visible text.

Return ONLY a JSON array. Each element:
{
  "text": "exact text content",
  "x0": 0.0,  // left edge as fraction of image width (0.0-1.0)
  "y0": 0.0,  // top edge as fraction of image height (0.0-1.0)  
  "x1": 1.0,  // right edge as fraction of image width (0.0-1.0)
  "y1": 1.0,  // bottom edge as fraction of image height (0.0-1.0)
  "role": "title|body|label|header|footer",
  "bold": true|false,
  "font_size_approx": 12.0
}

Rules:
- Include EVERY piece of visible text: headings, body text, labels, button text, captions, watermarks
- Include text inside colored buttons/badges (e.g. "Basic", "Silver", "Gold")  
- Include text inside diagrams and infographics
- Include text inside raster images embedded in the slide
- Do NOT include text from the QR code itself (just skip it)
- Bounding boxes must be accurate fractions (0.0 to 1.0)
- For multi-line text blocks, use ONE entry covering the whole block
- No markdown, no explanation — ONLY the JSON array"""
    ...
    raise ValueError(f"No text content in Claude response: {result}")


def _claude_vision_parse_blocks(
    raw_response: str,
    page: fitz.Page,
    page_index: int,
    min_chars: int = 2,
) -> List[Dict]:
    """Parse Claude Vision JSON response into OCR block dicts."""
    ...
    for item in items:
        ...
        raw_text = re.sub(r"\s+", " ", normalize_special_chars(str(item.get("text", "") or ""))).strip()
        if not should_keep_full_ocr_text_v16(raw_text, min_chars=min_chars):
            continue

        # Claude returns fractions 0.0–1.0.
        try:
            x0f = float(item.get("x0", 0.0))
            y0f = float(item.get("y0", 0.0))
            x1f = float(item.get("x1", 1.0))
            y1f = float(item.get("y1", 1.0))
        except (TypeError, ValueError):
            continue

        # Clamp and convert to PDF points.
        x0f, y0f, x1f, y1f = (
            max(0.0, min(1.0, x0f)),
            max(0.0, min(1.0, y0f)),
            max(0.0, min(1.0, x1f)),
        ...
        height_pt = max(1.0, pdf_bbox[3] - pdf_bbox[1])
        width_pt  = max(1.0, pdf_bbox[2] - pdf_bbox[0])

        # Use Claude's role hint; fall back to size heuristic.
        claude_role = str(item.get("role", "")).lower()
        if claude_role in {"title", "body", "label", "header", "footer"}:
            role = claude_role
        elif height_pt >= 20:
            role = "title"
        ...
        font_size = float(item.get("font_size_approx", 0.0))
        if font_size < 3.0:
            font_size = _estimate_font_size_v17(height_pt, raw_text, width_pt)

        block_cx = (pdf_bbox[0] + pdf_bbox[2]) / 2.0
        if abs(block_cx - page_cx) < page_w * 0.10 and width_pt < page_w * 0.5:
            align = "center"
        elif pdf_bbox[2] > page_w * 0.75 and pdf_bbox[0] > page_w * 0.45:
            align = "right"
        else:
            align = "left"

        # px_bbox is approximate (we don't have pixel coords from Claude).
        # Use None — cover_ocr_text_on_background_v16 will handle missing px_bbox.
        blocks.append({
            ...
            "bg_color": (1.0, 1.0, 1.0),  # will be sampled later
...
def ocr_page_to_all_line_blocks_claude_vision_v16(
    page: fitz.Page,
    page_index: int,
    api_key: str,
    base_url: str,
    ...
    """Extract all visible text using Claude Vision.

    Returns (pil_img, zoom, blocks) like the other OCR engines
    so it plugs into the same cover+draw pipeline.
    """
    from PIL import Image as _PILImage  # type: ignore

    zoom = dpi / 72.0

    # Render page to PIL image (for bg sampling and cover).
    mat = fitz.Matrix(zoom, zoom)
    pix = page.get_pixmap(matrix=mat, alpha=False)
    img = _PILImage.frombytes("RGB", [pix.width, pix.height], pix.samples)

    # Get base64 for API call.
    b64 = _claude_vision_page_to_b64(page, dpi=dpi)

    print(f"        Claude Vision API call (page {page_index + 1}) ...")
    try:
        raw = _claude_vision_call(
            b64_image=b64,
            api_key=api_key,
            base_url=base_url,
            model=model,
    ...
    # Sample actual background color for each block from the rendered image.
    page_w = float(page.rect.width)
    page_h = float(page.rect.height)
    for b in blocks:
        px_bbox = (
            max(0, int(b["bbox"][0] * zoom) - 2),
            max(0, int(b["bbox"][1] * zoom) - 2),
            int(b["bbox"][2] * zoom) + 2,
            int(b["bbox"][3] * zoom) + 2,
        )
        b["px_bbox"] = px_bbox
        b["bg_color"] = sample_bg_color_v17(img, px_bbox)
...
def full_ocr_rebuild_pdf_v16(
    ...
    Supported engines:
      tesseract — traditional OCR, fast, free
      paddle    — PaddleOCR, better Vietnamese support
      claude    — Claude Vision API, best quality for complex slides
                  (reads colored buttons, image-embedded text, diagrams)
    """
    # ── Engine setup ───────────────────────────────────────────
    ...
    # Claude Vision: reuse existing API credentials from .env
    claude_api_key = ""
    ...
    if ocr_engine == "claude":
        ...
        if not claude_api_key:
            raise ValueError(
                "Claude Vision engine requires LLM_API_KEY in .env "
                "(same key used for translation)"
            )
        # Override DPI for Claude Vision (lower is fine — Claude reads context, not pixels)
    ...
    try:
        for page_index, src_page in enumerate(src):
            ...
            print(f"      Full OCR page {page_index + 1}/{len(src)} [{ocr_engine}]")
            ...
            # ── Translate ──────────────────────────────────────
            # Claude Vision engine: Claude already read the text accurately.
            # Translation is still done by the configured LLM translator
            # (same as other engines) so Vietnamese output is consistent.
            if blocks:
            ...
            # ── Cover + render ─────────────────────────────────
            pad_px = max(1, int(dpi / 160))
            cleaned = cover_ocr_text_on_background_v16(img, blocks, pad_px=pad_px)
            ...
            print(f"        drawn={page_drawn}/{len(blocks)} blocks")
...
def build_arg_parser_v6() -> argparse.ArgumentParser:
    ...
    p.add_argument("--full-ocr-engine", default="tesseract", choices=["tesseract", "paddle", "claude"],
                   help="OCR engine for --full-ocr-rebuild. 'claude' uses Claude Vision API (best quality, reads image-embedded text)")
    p.add_argument("--full-ocr-lang", default="eng", help="OCR language for --full-ocr-rebuild (tesseract/paddle only)")
    ...
    p.add_argument("--claude-vision-model", default="claude-haiku-4-5",
                   help="Claude model for --full-ocr-engine claude (default: claude-haiku-4-5, cheap+fast)")
    p.add_argument("--claude-vision-dpi", type=int, default=200,
                   help="DPI for page rendering when using Claude Vision (default: 200)")
    return p
...
def main_v6(argv: Optional[List[str]] = None):
    ...
    if args.full_ocr_rebuild:
        print(f"[Full OCR rebuild] Input: {args.input_pdf} engine={args.full_ocr_engine}")
