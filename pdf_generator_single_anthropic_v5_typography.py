...
# -*- coding: utf-8 -*-
"""
pdf_generator_single_anthropic_fixed.py
...
Run:
    python pdf_generator_single_anthropic_fixed.py input.pdf output.pdf `
...
# VERSION_MARKER = 'single_anthropic_v3_no_temperature_by_default'
# VERSION_MARKER = 'single_anthropic_v5_typography_tuned'
# ==================== V8 CORE ====================
import argparse
import copy
import json
...
- Preserve product/technical names when they are placeholders or common product terms: CXVIEW, CXVIEW GPT Box, GPT Box, AI Video Analytics, CCTV, POS, API, RTSP, ONVIF, VMS, ROI, POC, PPE, USD.
...
- Keep CXVIEW, CXVIEW GPT Box, AI Video Analytics, CCTV, POS, API, RTSP, ONVIF, VMS, ROI, POC, PPE, USD unchanged.
- Use camera, not máy ảnh. Use địa điểm, not trang web. Use thanh toán, not hóa đơn.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--font-condensed", default=None, help="Condensed regular TTF/OTF font path")
    p.add_argument("--font-condensed-bold", default=None, help="Condensed bold TTF/OTF font path")
    p.add_argument("--font-medium", default=None, help="Medium TTF/OTF font path")
    p.add_argument("--font-semibold", default=None, help="SemiBold TTF/OTF font path")
    p.add_argument("--font-condensed-semibold", default=None, help="Condensed SemiBold TTF/OTF font path")
    p.add_argument("--font-black", default=None, help="Black/ExtraBold TTF/OTF font path")
    p.add_argument("--font-symbol", default=None, help="Symbol font path for checkmarks/bullets, e.g. C:/Windows/Fonts/seguisym.ttf")
...
def font_supports_text(path: Optional[str], text: str) -> bool:
    """Return True if font has glyphs for all important chars in `text`.

    This is stricter than font_supports_vietnamese(): a font may support Vietnamese
    but not the checkmark symbol, or vice versa.
    """
    ...
    try:
        ...
        for ch in text or "":
            if ch.isspace():
                continue
            # Ignore soft formatting/control chars.
...
def _font_family_stem(path: Optional[str]) -> str:
    if not path:
        return ""
    try:
...
class FontResolver:
    """Typography-tuned font resolver.

    V5 goals:
    - Actually use font-condensed / font-medium / font-black args in V9 render.
    - Soften body text weight. Many business PDFs extract Roboto-Medium even when
      visually it should be rendered closer to regular after translation.
    - Use condensed fonts for slide labels/table cells, but avoid over-bold labels
      when possible via SemiBold/Medium candidates.
    - Apply conservative role-based size calibration.
    """
    ...
    def __init__(
        ...
        self.condensed_regular_font = (
            self._resolve_existing_font(condensed_font)
            or self._discover_condensed_regular_from_paths([self.title_font, self.regular_font, self.bold_font])
            or self._windows_font("arialn.ttf")
            or self.regular_font
        )
        self.condensed_bold_font = (
            self._resolve_existing_font(condensed_bold_font)
            or self._discover_condensed_bold_from_paths([self.title_font, self.bold_font, self.regular_font])
            or self.title_font
            or self._windows_font("arialnb.ttf")
            or self.bold_font
        )
        self.semibold_font = (
            self._resolve_existing_font(semibold_font)
            or self._discover_variant(["SemiBold", "Semibold", "DemiBold", "Demi"], [self.regular_font, self.bold_font, self.title_font])
            or self.bold_font
        )
        self.medium_font = (
            self._resolve_existing_font(medium_font)
            or self._discover_variant(["Medium"], [self.regular_font, self.bold_font, self.title_font])
            or self.semibold_font
            or self.bold_font
        )
        self.condensed_semibold_font = (
            self._resolve_existing_font(condensed_semibold_font)
            or self._discover_condensed_semibold_from_paths([self.condensed_bold_font, self.condensed_regular_font, self.title_font])
            or self.condensed_bold_font
            or self.semibold_font
        )
        self.black_font = (
            self._resolve_existing_font(black_font)
            or self._discover_variant(["Black", "ExtraBold", "Heavy"], [self.title_font, self.bold_font, self.regular_font])
            or self.condensed_bold_font
            or self.bold_font
        )
        self.symbol_font = (
            self._resolve_existing_font(symbol_font)
            or self._windows_font("seguisym.ttf")
            or self._windows_font("arial.ttf")
            or self.regular_font
        )
        ...
        self.prefer_original_fonts = prefer_original_fonts
        self._text_support_cache: Dict[Tuple[str, str], bool] = {}
        self._font_cache: Dict[str, fitz.Font] = {}
        self._scale_cache: Dict[Tuple[str, str, str], float] = {}

        print("FontResolver v5 typography:")
    ...
    def _discover_variant(self, style_names: List[str], bases: Optional[List[Optional[str]]] = None) -> Optional[str]:
        candidates: List[Path] = []
        for base in (bases or [self.regular_font, self.bold_font, self.title_font]):
            if not base:
                continue
            p = Path(base)
            parent = p.parent
    ...
    def _discover_condensed_regular_from_paths(self, bases: List[Optional[str]]) -> Optional[str]:
        candidates: List[Path] = []
        for base in bases:
            if not base:
                continue
            ...
            parent = p.parent
            suffix = p.suffix
            candidates.extend(parent / f"{n}{suffix}" for n in [
                "NotoSansCondensed-Regular",
                "NotoSansCondensed",
                "RobotoCondensed-Regular",
                "RobotoCondensed",
            ])
            candidates.append(p.with_name(re.sub(r"Bold|SemiBold|Semibold|Black|Medium|ExtraBold|Heavy", "Regular", p.name)))
        for c in candidates:
            if c.exists():
                return str(c)
        return None

    def _discover_condensed_semibold_from_paths(self, bases: List[Optional[str]]) -> Optional[str]:
        candidates: List[Path] = []
        for base in bases:
            if not base:
                continue
            ...
            suffix = p.suffix
            candidates.extend(parent / f"{n}{suffix}" for n in [
                "NotoSansCondensed-SemiBold",
                "NotoSansCondensed-Semibold",
                "NotoSansCondensed-Medium",
                "RobotoCondensed-SemiBold",
                "RobotoCondensed-Medium",
            ])
            candidates.append(p.with_name(re.sub(r"Bold|Black|ExtraBold|Heavy", "SemiBold", p.name)))
            candidates.append(p.with_name(re.sub(r"Bold|Black|ExtraBold|Heavy", "Medium", p.name)))
        for c in candidates:
            if c.exists():
                return str(c)
        return None

    def _discover_condensed_bold_from_paths(self, bases: List[Optional[str]]) -> Optional[str]:
        candidates: List[Path] = []
        for base in bases:
            if not base:
                continue
    ...
    def _supports_text_cached(self, path: Optional[str], text: str) -> bool:
        if not path:
            return False
        key = (path, text or "")
        if key not in self._text_support_cache:
            self._text_support_cache[key] = font_supports_text(path, text or "")
        return self._text_support_cache[key]

    def _style_flags(self, block: TextBlock) -> Dict[str, bool]:
        name = clean_font_name(block.main_font).lower()
        role0 = getattr(block, "role", "body")
        small = is_small_box(block) if "is_small_box" in globals() else False
        role = "label" if small else role0
        fs = float(block.font_size or 0)

        explicit_black = any(x in name for x in ["black", "heavy", "extrabold"])
        explicit_bold = "bold" in name or explicit_black
        explicit_medium = any(x in name for x in ["medium", "semibold", "semi-bold", "demibold"])
        is_body = role == "body"

        return {
            "role": role,
            "small": small,
            "condensed": ("condensed" in name or "narrow" in name or role in {"label", "table", "table_cell"} or small),
            "black": explicit_black,
            "medium": explicit_medium and not (is_body and not explicit_bold),
            "bold": bool(block.is_bold or explicit_bold or (explicit_medium and role in {"label", "table", "table_cell", "title", "cta"})),
            "title": role == "title" or fs >= 18,
            "symbol": any(ch in (block.translated_text or block.original_text or "") for ch in "✓✔☑ü"),
        }

    def _label_weight_mode(self) -> str:
        return os.getenv("PDF_TRANSLATOR_LABEL_WEIGHT", "semibold").strip().lower()

    def _candidate_fonts_for_block(self, block: TextBlock, text: Optional[str] = None) -> List[Tuple[str, Optional[str]]]:
        flags = self._style_flags(block)
        role = str(flags["role"])
        candidates: List[Tuple[str, Optional[str]]] = []

        def add(label: str, path: Optional[str]):
            if path and (label, path) not in candidates:
                candidates.append((label, path))

        if flags["symbol"]:
            add("symbol", self.symbol_font)

        if role == "body":
            if flags["black"]:
                add("black", self.black_font)
            elif flags["bold"]:
                add("semibold", self.semibold_font)
                add("bold", self.bold_font)
            else:
                add("regular", self.regular_font)
                add("medium", self.medium_font)
            add("regular", self.regular_font)
            add("bold", self.bold_font)
            return candidates
        ...
        if flags["title"]:
            add("black", self.black_font)
            add("title", self.title_font)
            add("condensed_bold", self.condensed_bold_font)
            add("semibold", self.semibold_font)
            add("bold", self.bold_font)

        if flags["condensed"]:
            mode = self._label_weight_mode()
            if mode == "regular":
                add("condensed_regular", self.condensed_regular_font)
                add("condensed_semibold", self.condensed_semibold_font)
                add("condensed_bold", self.condensed_bold_font)
            elif mode == "bold":
                add("condensed_bold", self.condensed_bold_font)
                add("condensed_semibold", self.condensed_semibold_font)
    ...
    def _fallback_font_for_block(self, block: TextBlock, text: Optional[str] = None) -> Optional[str]:
        render_text = text or getattr(block, "_selected_text_for_font", None) or block.translated_text or block.original_text or ""
        for _, path in self._candidate_fonts_for_block(block, render_text):
            if path and self._supports_text_cached(path, render_text):
                return path

        relaxed = render_text.replace("✓", "").replace("✔", "").replace("☑", "").replace("ü", "")
        if relaxed != render_text:
            for _, path in self._candidate_fonts_for_block(block, relaxed):
                if path and self._supports_text_cached(path, relaxed):
                    return path
    ...
    def fontfile_for(self, block: TextBlock, text: Optional[str] = None) -> Optional[str]:
        ...
        if self.prefer_original_fonts and original and self._supports_text_cached(original, render_text):
            return original
        return self._fallback_font_for_block(block, render_text)
    ...
    def fontname_for(self, block: TextBlock, text: Optional[str] = None) -> str:
        ...
        if chosen == self.symbol_font and flags["symbol"]:
            return "FSymbolVN"
        if chosen == self.black_font:
            return "FBlackVN"
        if chosen == self.condensed_bold_font:
            return "FCondensedBoldVN"
        if chosen == self.condensed_semibold_font:
            return "FCondensedSemiBoldVN"
        if chosen == self.condensed_regular_font:
            return "FCondensedRegularVN"
        if chosen == self.semibold_font:
            return "FSemiBoldVN"
        if chosen == self.medium_font:
            return "FMediumVN"
        if chosen == self.title_font or flags["title"]:
            return "FTitleVN"
        if chosen == self.bold_font or "bold" in name:
            return "FBoldVN"
        return "FRegularVN"
    ...
    def _role_size_adjustment(self, block: TextBlock) -> float:
        flags = self._style_flags(block)
        role = str(flags["role"])
        global_scale = self._env_float("PDF_TRANSLATOR_SIZE_SCALE_GLOBAL", 1.0)

        if role == "title":
            role_scale = self._env_float("PDF_TRANSLATOR_SIZE_SCALE_TITLE", 1.00)
        elif role in {"label", "table", "table_cell"} or flags["small"]:
            role_scale = self._env_float("PDF_TRANSLATOR_SIZE_SCALE_LABEL", 1.06)
        elif role == "cta":
            role_scale = self._env_float("PDF_TRANSLATOR_SIZE_SCALE_CTA", 1.02)
        else:
            role_scale = self._env_float("PDF_TRANSLATOR_SIZE_SCALE_BODY", 0.96)

        fs = float(block.font_size or 0)
        if role == "body" and fs <= 6.8:
            role_scale *= self._env_float("PDF_TRANSLATOR_SIZE_SCALE_TINY_BODY", 0.96)

        return global_scale * role_scale
    ...
    def font_size_scale_for(self, block: TextBlock, text: Optional[str] = None) -> float:
        ...
        if fallback_path and original_path and fallback_path != original_path:
            ...
            if key in self._scale_cache:
                ...
                try:
                    ...
                    flags = self._style_flags(block)
                    if flags["small"] or flags["role"] in {"label", "table", "table_cell"}:
                        lo, hi = 0.88, 1.10
                    elif flags["title"]:
                        lo, hi = 0.88, 1.06
                    else:
        ...
        scale = base_scale * self._role_size_adjustment(block)
        flags = self._style_flags(block)
        if flags["small"] or flags["role"] in {"label", "table", "table_cell"}:
            return max(0.88, min(1.14, scale))
        if flags["role"] == "body":
            return max(0.86, min(1.04, scale))
        return max(0.86, min(1.10, scale))
...
def compact_override_for_block(block: TextBlock, target_lang: str = "vi") -> Optional[str]:
    ...
    # Fall back to base exact memory.
    exact = exact_translation_override(block.original_text, target_lang)
    if exact:
        return exact
    return None
...
def has_english_leak(text: str, original_text: str = "", role: str = "body") -> bool:
    # Protected uppercase acronyms/products are OK. Lowercase/Titlecase English words are suspicious in Vietnamese output.
    ...
    # In body text, a few technical/proper English terms may remain, but labels should be clean.
    if role in {"label", "table", "table_cell", "title", "cta"}:
    ...
    for start in range(0, len(pending), batch_size):
        ...
        for block in batch:
            ...
            payload.append({
                ...
                "instruction": "Use compact Vietnamese. Return only the translated string for this block. No NBSP. No English except protected product/technical terms.",
            })
...
def role_policy_v9(block: TextBlock) -> Tuple[float, List[float], int]:
    ...
    if role == "title":
        return float(os.getenv("PDF_TRANSLATOR_MIN_SCALE_TITLE", "0.88")), [1.00, 0.96, 0.92], 1
    if role in {"label", "table", "table_cell"}:
        return float(os.getenv("PDF_TRANSLATOR_MIN_SCALE_LABEL", "0.86")), [1.02, 0.98, 0.94, 0.90], 1
    if role == "cta":
        return float(os.getenv("PDF_TRANSLATOR_MIN_SCALE_CTA", "0.86")), [1.02, 0.98, 0.94], 1
    return float(os.getenv("PDF_TRANSLATOR_MIN_SCALE_BODY", "0.84")), [1.08, 1.04, 1.00, 0.96], 2
...
def compute_layout_v9(text: str, block: TextBlock, rect: fitz.Rect, font: fitz.Font) -> LayoutResult:
    ...
    size_scale = float(getattr(block, "_font_size_scale", 1.0) or 1.0)
    original_size = max(4.8, float(block.font_size or 8.0) * size_scale)
...
def render_translated_pdf_v9(
    ...
    for page_ir in translated_ir.pages:
        ...
        for block in page_ir.blocks:
            ...
            # Choose closest Vietnamese-capable fallback font by original style.
            # Also compute font-size scaling from source/fallback font metrics.
            ...
            # Mutate the layout/font/size to match the selected compact candidate if changed.
            if chosen_text != raw_text:
...
class ProviderAwareTranslator(_OriginalOpenAICompatibleTranslator):
    """
    Single-file provider-aware translator.

    If:
        LLM_PROVIDER=anthropic

    It calls Anthropic native Messages API:
        POST {LLM_BASE_URL}/v1/messages

    Otherwise it falls back to the original OpenAI-compatible /chat/completions flow.
    """
    ...
    def _anthropic_messages_completion(self, system_prompt: str, user_prompt: str) -> str:
        ...
        # Anthropic newer/high-tier models can reject `temperature`.
        # Default: do NOT send temperature. This avoids:
        #   invalid_request_error: "`temperature` is deprecated for this model."
        #
        # If you really need it for a model that supports temperature, set:
        ...
        headers = {
            ...
            "accept": "application/json",
            "user-agent": os.getenv("LLM_USER_AGENT", "pdf-translator-mikotech/1.0 anthropic-native"),
        }
