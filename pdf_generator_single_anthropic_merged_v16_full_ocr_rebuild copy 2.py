...
def apply_cxview_manual_patches_v13(
    ...
    patches = [
        ...
        (3, (648, 129, 777, 152.5), "ĐỘ TRỄ GẦN 0", (0.45, 0.23, 0.64), (1, 1, 1), 7.6, "bold"),
        (3, (648, 156, 777, 179.5), "CHỦ QUYỀN DỮ LIỆU", (0.45, 0.23, 0.64), (1, 1, 1), 7.0, "bold"),
        (3, (648, 183, 777, 206.5), "TỐI ƯU CHI PHÍ", (0.45, 0.23, 0.64), (1, 1, 1), 7.5, "bold"),
        (3, (648, 210, 777, 233.5), "TIẾT KIỆM BĂNG THÔNG", (0.45, 0.23, 0.64), (1, 1, 1), 6.7, "bold"),
    ]
...
# ============================================================
# CLAUDE VISION REGION PATCH ENGINE
# ============================================================
# Hybrid approach:
#   1. Take the already-translated PDF (native pipeline output)
#   2. For each specified image region, render that region to PNG
#   3. Send PNG to Claude Vision → get text + bbox (fractional)
#   4. Translate extracted text via LLM
#   5. Cover original text + draw Vietnamese via V26.3 patch engine
#
# This preserves native pipeline quality and only adds Claude
# Vision for the parts that couldn't be translated (image-embedded
# text like badges, diagrams, infographics).


def _cvr_render_region_b64(
    page: fitz.Page,
    region: fitz.Rect,
    dpi: int = 220,
) -> Tuple[str, float]:
    """Render a specific page region to base64 PNG."""
    import base64
    zoom = dpi / 72.0
    clip = fitz.Rect(
        max(0, region.x0), max(0, region.y0),
...
def _cvr_call_claude(
    ...
    """Ask Claude to extract text from a region image.

    Returns list of dicts: {text, x0, y0, x1, y1} where coords
    are fractions of the region (not the full page).
    """
    prompt = (
        "Extract all visible text from this image region of a PDF slide. "
        "Return ONLY a JSON array. Each element: "
        '{"text": "...", "x0": 0.0, "y0": 0.0, "x1": 1.0, "y1": 1.0} '
        "where coordinates are fractions (0.0-1.0) of THIS image's width/height. "
        "Include text in buttons, badges, labels, captions. "
        "Skip decorative icons and QR codes. "
    ...
    try:
        ...
        try:
            ...
            s = repaired.find("[")
            items = json.loads(repaired[s:]) if s >= 0 else []
        except Exception:
            print(f"        Claude Vision JSON parse error: {ex}")
            return []

    # Convert fractional region coords → PDF page coords.
    rw = region.width
    rh = region.height
    results = []
    for item in items:
        ...
        if not text or len(text) < 2:
            continue
        try:
            x0f = max(0.0, min(1.0, float(item.get("x0", 0.0))))
            y0f = max(0.0, min(1.0, float(item.get("y0", 0.0))))
            x1f = max(0.0, min(1.0, float(item.get("x1", 1.0))))
            y1f = max(0.0, min(1.0, float(item.get("y1", 1.0))))
        except (TypeError, ValueError):
            continue
        if x1f <= x0f or y1f <= y0f:
            continue
        # Convert to page PDF points.
        pdf_bbox = (
            region.x0 + x0f * rw,
            region.y0 + y0f * rh,
            region.x0 + x1f * rw,
        ...
        results.append({"text": text, "bbox": pdf_bbox})

    return results


def _cvr_translate_blocks(
    blocks: List[Dict],
    translator,
    source_lang: str,
    target_lang: str,
    ...
    """Translate Claude Vision extracted blocks using existing translator."""
    pending = []
    for i, b in enumerate(blocks):
        # Try compact translation memory first.
        class _Proxy:
            def __init__(self, blk):
                self.original_text = blk["text"]
                self.role = "label"
                self.font_size = max(4.0, (blk["bbox"][3]-blk["bbox"][1]) * 0.72)
                self.bbox = blk["bbox"]
                self.lines = [type("L", (), {"text": blk["text"]})()]
        override = compact_override_for_block(_Proxy(b), target_lang)
        if override:
            b["translated"] = sanitize_text_v9(override, b["text"], "label")
        else:
            pending.append((i, b))
    ...
    for start in range(0, len(pending), batch_size):
        ...
        items = [{
            ...
            "max_chars": 60,
            "max_lines": 3,
            "instruction": "Translate this visible text into the target language. Keep brand names unchanged.",
        } for idx, b in batch]
        results = translator.translate_batch(items, source_lang, target_lang, glossary)
        by_id = {x["id"]: x.get("translated", "") for x in results}
        for idx, b in batch:
            raw = by_id.get(str(idx), b["text"])
            b["translated"] = sanitize_text_v9(raw, b["text"], "label")
...
def apply_claude_vision_region_patch(
    ...
    """Main entry: Claude Vision reads image regions, translates, patches PDF.

    Args:
        source_pdf: Original source PDF (for image rect detection).
        translated_pdf: Already-translated PDF from native pipeline.
        output_pdf: Final output path.
        image_region_map: JSON file defining regions to process.
    """
    # Load regions.
    manual_regions = _v263_load_region_map(image_region_map)
    auto_regions = _v263_auto_image_rects(source_pdf, min_area=800.0)

    all_regions: Dict[int, List[fitz.Rect]] = {}
    for m in (auto_regions, manual_regions):
        for pn, rects in m.items():
            all_regions.setdefault(pn, []).extend(rects)

    if not all_regions:
        print("      Claude Vision Region Patch: no regions defined")
        return 0
    ...
    for page_index, page in enumerate(pdf):
        page_num = page_index + 1
        regions = all_regions.get(page_num, [])
        if not regions:
            continue

        print(f"      Claude Vision Region page {page_num}: {len(regions)} regions")
        ...
        for region in regions:
            ...
            if debug:
                page.draw_rect(region, color=(0, 0.6, 0), width=1.0, overlay=True)

            b64, zoom = _cvr_render_region_b64(page, region, dpi=dpi)
            extracted = _cvr_call_claude(
                b64, region, api_key, base_url, model,
                anthropic_version=anthropic_version,
            )
            print(f"        Region [{region.x0:.0f},{region.y0:.0f},{region.x1:.0f},{region.y1:.0f}]: {len(extracted)} text blocks")
            all_blocks.extend(extracted)

        if not all_blocks:
            continue

        # Translate all blocks from this page.
        all_blocks = _cvr_translate_blocks(
            all_blocks, translator, source_lang, target_lang,
            glossary=glossary, batch_size=batch_size,
        )

        # Draw each translated block.
        for b in all_blocks:
            text_src  = b.get("text", "")
            text_vi   = b.get("translated", "")
            if not text_vi or text_is_same(text_src, text_vi):
                continue
            ...
            # Sample background, cover original, draw translation.
            bg = _v263_sample_bg(page, rect)
            lum = _v263_luminance(bg)
            sat = max(bg[0], bg[1], bg[2]) - min(bg[0], bg[1], bg[2])

            # Cover original text.
            page.draw_rect(rect, color=None, fill=bg, overlay=True)

            # Choose text color.
            color = (1.0, 1.0, 1.0) if (lum < 0.50 or sat > 0.15) else (0.06, 0.06, 0.10)
            ...
            ht = max(1.0, rect.height)
            wt = max(1.0, rect.width)
            font_size = _estimate_font_size_v17(ht, text_vi, wt)

            size = font_size
            min_size = max(3.5, font_size * 0.55)
            while size >= min_size:
                try:
                    rc = page.insert_textbox(
                        rect, text_vi,
                        fontsize=size, fontname=fontname, fontfile=fontfile,
                        color=color, align=fitz.TEXT_ALIGN_CENTER, overlay=True,
                    )
                    if rc >= 0:
                        break
                except Exception:
    ...
    # Save with temp-file swap (same as V26.3 to avoid same-path error).
    import shutil as _shutil
    out_path = Path(output_pdf).resolve()
    tr_path  = Path(translated_pdf).resolve()
    if tr_path == out_path:
        tmp_path = str(out_path.parent / (out_path.stem + "_cvrpatch_tmp.pdf"))
        pdf.save(tmp_path, garbage=4, deflate=True)
        pdf.close()
        _shutil.move(tmp_path, str(out_path))
    else:
    ...
    print(f"      Claude Vision Region Patch: {total_patches} patches applied → {output_pdf}")
...
def build_arg_parser_v6() -> argparse.ArgumentParser:
    ...
    # V26.3 inline patch — runs at the END of the pipeline automatically.
    ...
    # Claude Vision Region Patch: hybrid approach — native pipeline + Claude Vision for image regions only.
    p.add_argument("--cv-region-patch", action="store_true",
                   help="After translation, use Claude Vision to read/translate text inside image regions only")
    p.add_argument("--cv-region-map", default=None,
                   help="Image region map JSON for Claude Vision region patch (same format as --v263-image-region-map)")
    p.add_argument("--cv-model", default="claude-haiku-4-5",
                   help="Claude model for --cv-region-patch (default: claude-haiku-4-5)")
    p.add_argument("--cv-dpi", type=int, default=220,
                   help="DPI for region rendering in --cv-region-patch (default: 220)")
    ...
    p.add_argument("--full-ocr-engine", default="tesseract", choices=["tesseract", "paddle"], help="Engine for --full-ocr-rebuild")
...
def main_v6(argv: Optional[List[str]] = None):
    ...
    # Claude Vision Region Patch — hybrid: native pipeline + Claude Vision for image regions.
    if getattr(args, "cv_region_patch", False):
        _cv_out = args.output_pdf if not getattr(args, "ocr_output", None) else args.ocr_output
        _cv_api_key  = os.getenv("LLM_API_KEY", "").strip()
        _cv_base_url = os.getenv("LLM_BASE_URL", "https://api.anthropic.com").strip().rstrip("/")
        _cv_version  = os.getenv("ANTHROPIC_VERSION", "2023-06-01").strip()
        _cv_model    = getattr(args, "cv_model", "claude-haiku-4-5")
        _cv_dpi      = getattr(args, "cv_dpi", 220)
        _cv_region   = getattr(args, "cv_region_map", None)
        _cv_debug    = getattr(args, "cv_debug", False)

        if not _cv_api_key:
            print("WARNING: --cv-region-patch requires LLM_API_KEY in .env — skipping")
        else:
            print(f"[CV Region Patch] model={_cv_model} dpi={_cv_dpi}")
            apply_claude_vision_region_patch(
                source_pdf=args.input_pdf,
                translated_pdf=_cv_out,
                output_pdf=_cv_out,
                image_region_map=_cv_region,
                translator=translator,
                source_lang=args.source,
                target_lang=args.target,
                glossary=glossary,
                api_key=_cv_api_key,
                base_url=_cv_base_url,
                model=_cv_model,
                anthropic_version=_cv_version,
                dpi=_cv_dpi,
                font_regular=args.font,
    ...
    # V26.3 inline region patch — runs after the full pipeline with no extra script.
    ...
    if getattr(args, "v263_patch_map", None) and Path(args.v263_patch_map).exists():
        ...
        print(f"[V26.3] Applied {n_patches} patches → {_v263_out}")
