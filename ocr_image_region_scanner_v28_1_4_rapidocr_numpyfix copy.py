...
# -*- coding: utf-8 -*-
"""
V28.1.4 OCR Image-Region Scanner for PDF translation patches.
...
def _to_plain_list_v2814(value: Any) -> List[Any]:
    """Safely convert ndarray/tuple/list/None to a Python list.

    Important: never use `array or []` because numpy arrays raise
    `ValueError: The truth value of an array is ambiguous` and caused v28.1.3
    to silently fall through to zero candidates for RapidOCR 2.x outputs.
    """
    if value is None:
        return []
...
def normalize_ocr_result(raw: Any) -> List[Tuple[List[List[float]], str, float]]:
    """Normalize OCR result into [(quad, text, confidence 0-1), ...].

    Supports PaddleOCR, RapidOCR legacy, and RapidOCR 2.x object output.

    v28.1.4 fix:
    - Handles numpy arrays in RapidOCROutput.boxes/scores without boolean `or []`.
    - Supports alternate attribute names: rec_texts, rec_scores, dt_boxes.
    - Supports list of dicts / tuple rows.
    """
    out: List[Tuple[List[List[float]], str, float]] = []
    if raw is None:
        return out

    # RapidOCR 2.x / newer output objects.
    if any(hasattr(raw, attr) for attr in ("boxes", "txts", "scores", "rec_texts", "rec_scores", "dt_boxes")):
        boxes = _to_plain_list_v2814(
            getattr(raw, "boxes", None)
            if getattr(raw, "boxes", None) is not None
            else getattr(raw, "dt_boxes", None)
        )
        texts = _to_plain_list_v2814(
            getattr(raw, "txts", None)
            if getattr(raw, "txts", None) is not None
            else getattr(raw, "rec_texts", None)
        )
        scores = _to_plain_list_v2814(
            getattr(raw, "scores", None)
            if getattr(raw, "scores", None) is not None
            else getattr(raw, "rec_scores", None)
        )
        # Some versions use rec_boxes for rectangular boxes.
        if not boxes:
            boxes = _to_plain_list_v2814(getattr(raw, "rec_boxes", None))
        if boxes and texts:
            if not scores:
                scores = [1.0] * len(texts)
            for box, text, score in zip(boxes, texts, scores):
                quad = box_to_quad(box)
                if quad and str(text).strip():
                    try:
                        score_f = float(score)
                    except Exception:
                        score_f = 0.0
                    out.append((quad, str(text), score_f))
            return out

    # Convert object to dict if available.
    for meth in ("to_dict", "dict", "as_dict"):
        if hasattr(raw, meth):
            try:
                maybe = getattr(raw, meth)()
    ...
    # Common older API: (ocr_result, infer_elapse) should have been unwrapped,
    # but support it defensively.
    if isinstance(raw, tuple) and len(raw) >= 1:
        raw = raw[0]

    if isinstance(raw, dict):
        texts = _to_plain_list_v2814(raw.get("rec_texts") or raw.get("txts") or raw.get("texts"))
        scores = _to_plain_list_v2814(raw.get("rec_scores") or raw.get("scores") or raw.get("confidences"))
        boxes = _to_plain_list_v2814(raw.get("rec_boxes") or raw.get("boxes") or raw.get("dt_boxes") or raw.get("points"))
        if not scores:
            scores = [1.0] * len(texts)
        for box, text, score in zip(boxes, texts, scores):
            quad = box_to_quad(box)
    ...
    if isinstance(raw, list) and len(raw) == 1 and isinstance(raw[0], list):
        # Paddle page wrapper: [[box, (text, score)], ...]
    ...
    for item in candidates:
        try:
            if isinstance(item, dict):
                ...
                quad = box_to_quad(item.get("box") or item.get("bbox") or item.get("points") or item.get("dt_box"))
                if quad and str(text).strip():
...
def scan_regions(
            ...
            for variant_name, vimg in variants.items():
                ...
                try:
                    if active_engine_name == "rapidocr":
                        raw = run_rapidocr(ocr, str(tmp_path), box_thresh=0.10, text_score=max(0.02, min_confidence * 0.35))
                    else:
                        try:
                ...
                for quad, text, score in normalize_ocr_result(raw):
                    ...
                    region_items.append(OCRItem(
                        ...
                        detection_count=1,
                        english_like=True,
                        source=f"{active_engine_name}_v28_1_4",
                    ))
    ...
    pdf.close()
    report = {
        "version": "v28_1_4_ocr_image_region_report",
        "input_pdf": pdf_path,
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print("V28.1.4 OCR Image-Region Scanner summary:")
