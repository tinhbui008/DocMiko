...
# VERSION_MARKER = 'single_anthropic_v4_fontfix_metric_fallback'
# ==================== V8 CORE ====================
import argparse
import copy
import json
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--font-medium", default=None, help="Medium/SemiBold TTF/OTF font path")
...
class FontResolver:
    """Font resolver v10/v4: closest Vietnamese-capable fallback.

    Strategy:
    - Prefer original extracted embedded font only when it supports the actual
      translated text.
    - Otherwise fallback by style: condensed / medium / bold / black / symbol.
    - Compute font-size scale from original-font metrics to fallback-font metrics
      so the rendered text visually matches the source PDF more closely.
    """
    ...
    def __init__(
        ...
        self.medium_font = (
            ...
            or self._discover_variant(["Medium", "SemiBold", "Semibold"], [self.regular_font, self.bold_font, self.title_font])
            or self.bold_font
        )
        self.black_font = (
            self._resolve_existing_font(black_font)
            or self._discover_variant(["Black", "ExtraBold", "Heavy", "Bold"], [self.title_font, self.bold_font, self.regular_font])
    ...
    def _discover_condensed_regular_from_paths(self, bases: List[Optional[str]]) -> Optional[str]:
        ...
        for base in bases:
            ...
            names = [
                ...
                "NotoSansCondensed",
                "NotoSansDisplay-CondensedRegular",
                "NotoSansDisplay-Condensed-Regular",
                "RobotoCondensed-Regular",
                "RobotoCondensed",
            ]
            candidates.extend(parent / f"{n}{suffix}" for n in names)

            # If title is NotoSansCondensed-Bold.ttf, derive Regular.
    ...
    def _discover_condensed_bold_from_paths(self, bases: List[Optional[str]]) -> Optional[str]:
        ...
        for base in bases:
            ...
            names = [
                ...
                "NotoSansDisplay-Condensed-Bold",
    ...
    def _style_flags(self, block: TextBlock) -> Dict[str, bool]:
        ...
        return {
            ...
            "black": ("black" in name or "heavy" in name or "extrabold" in name),
            "medium": ("medium" in name or "semibold" in name),
            "bold": bool(block.is_bold or "bold" in name or "medium" in name or "black" in name or "heavy" in name),
    ...
    def _fallback_font_for_block(self, block: TextBlock, text: Optional[str] = None) -> Optional[str]:
        ...
        # Choose the first visually suitable font that supports the actual text.
        for _, path in self._candidate_fonts_for_block(block, render_text):
            if path and self._supports_text_cached(path, render_text):
                return path

        # Relax symbol/checkmark if no font supports everything.
    ...
    def fontfile_for(self, block: TextBlock, text: Optional[str] = None) -> Optional[str]:
        ...
        # Use original only if it supports the exact text. Subset PDF fonts often do not.
    ...
    def fontname_for(self, block: TextBlock, text: Optional[str] = None) -> str:
        ...
        if chosen == self.title_font or flags["title"]:
            return "FTitleVN"
        if chosen == self.bold_font or block.is_bold or "bold" in name:
            return "FBoldVN"
        return "FRegularVN"
    ...
    def font_size_scale_for(self, block: TextBlock, text: Optional[str] = None) -> float:
        """Scale fallback font size to better match the source font metrics.

        If the fallback font is wider than the source embedded font, reduce size
        slightly. If narrower, allow a tiny increase. Keep bounds conservative.
        """
        ...
        key = (clean_font_name(block.main_font), fallback_path, render_text[:80])
        ...
        try:
            ...
            # Prefer source Latin text for original subset font measurement. It is
            # almost always supported by the embedded subset.
            ...
            # Conservative clamps. Avoid huge visual jumps.
            if flags["condensed"] or is_small_box(block):
                lo, hi = 0.82, 1.06
            elif flags["title"]:
                lo, hi = 0.86, 1.04
            else:
            ...
            # Do not enlarge body text too much; it quickly looks off in slides.
            if getattr(block, "role", "body") == "body":
...
def role_policy_v9(block: TextBlock) -> Tuple[float, List[float], int]:
    ...
    if role == "title":
        return 0.82, [1.00, 0.96, 0.92, 0.88], 1
    if role in {"label", "table", "table_cell"}:
        return 0.72, [0.98, 0.94, 0.90, 0.86], 1
    if role == "cta":
        return 0.78, [1.02, 0.98, 0.94, 0.90], 1
